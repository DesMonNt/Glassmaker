...
namespace FightingScene.Units
{
    ...
    {
        public ShapeShell() : 
            base(new UnitStats(7000, 0.2f, 200, 90, false, 0.1f, TypeOfAttack.Single, 0))
        {
            Brain = new HealerAI(this);
            Skill = new Ability(new List<IBuff>(), new List<IBuff> { new ShieldBuff((int)(0.13 * CurrentStats.MaxHealth)) }, "Оберегающий панцирь");
        ...
        public override Ability UseAbility() => new (new List<IBuff>(),
            new List<IBuff> { new ShieldBuff((int)(0.13 * CurrentStats.MaxHealth)) }, "Оберегающий панцирь");
